...
class Manager:
    ...
    def __parse_args(self) -> None:
        ...
        parser.add_argument('-pa', '--path', metavar="Custom path to the towers' file.", default="scripts/.towers_pos", dest="path", help="Custom path to the towers' file.", type=str)
    ...
    def __generate(self) -> None:
        for i in range(self.__data.number):
            ...
            self.__save_file(f"scripts/{self.__planes_nb[i]}_planes_{self.__towers_nb[i]}_towers.rdr")
            self.__towers = []
            self.__towers_ = []
            self.__planes = []
